...
const ProfileImage = ({ call }: ProfileImageProps) => {
    const shortName = `${call.firstName.charAt(0)}${call.lastName.charAt(
        0
    )}`;
    ...
    const displayNumbers = call.peoplesAvailableOnCall > 2 ? call.peoplesAvailableOnCall - 1 : 0;
    const displayProfile = call.peoplesAvailableOnCall > 2 ?
        (call.peoples && call.peoples.length > 0) ? call.peoples[0] : null : null;
    const groupProfile = displayProfile ? `${displayProfile.firstName.charAt(0)}${displayProfile.lastName.charAt(
        0
    )}` : "";
        ...
        <>
            {
                call.peoplesAvailableOnCall > 2 ?
                                ...
                                <div className="avatar-xs">
                                    {
                                        displayProfile && displayProfile.profileImage ?
                                            <img src={displayProfile.profileImage} alt="" className="img-fluid rounded-circle" />
                                            :
                                            <span
                    ...
                    : call.profileImage ?
                        <div className="chat-user-img flex-shrink-0 me-2">
                            <img src={call.profileImage} className="rounded-circle avatar-xs" alt="" />
                        </div>
                        :
                ...
                <div className="d-flex align-items-center">
                    ...
                    <div className="flex-shrink-0 ms-3">
                        <div className="d-flex align-items-center gap-3">
                            <div>
                                <h5 className="mb-0 font-size-12 text-muted">{call.callDuration}</h5>
                            </div>
                            <div>
                                {
                                    call.hasVideoCall ?
                                        <Button color="link" type="button" className="p-0 font-size-20 stretched-link" onClick={onOpenVideo}>
                                            <i className="bx bx-video align-middle"></i>
                                        </Button>
                                        :
                                        <Button color="link" type="button" className="p-0 font-size-20 stretched-link" onClick={onOpenAudio}>
