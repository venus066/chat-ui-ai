...
const FormCheck = ({ color, selected, onChange }: FormCheckProps) => {
    ...
    return (
        <div className="form-check">
            <input className="form-check-input theme-color"
                type="radio"
                name="bgcolor-radio" id={color.id}
                onChange={() => {
                    onChange(color);
                }}
                checked={checked}
            ...
            <label className="form-check-label avatar-xs" htmlFor={color.id}>
                <span className={
                    classnames("avatar-title", "rounded-circle", "theme-btn", [color.color], [color.id])
                }
                    id={`color-${color.id}`}
                >
                </span>
...
const ThemeColor = ({ theme }: ThemeColorProps) => {
    ...
    useEffect(() => {
        if (theme && theme.color) {
            const userTheme = themeColors.find((c: ThemeColorTypes) => c.id === theme.color);
            if (userTheme) {
