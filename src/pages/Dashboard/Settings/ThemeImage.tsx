...
const FormCheck = ({ image, selected, onChange }: FormCheckProps) => {
    ...
    return (
        <div className="form-check">
            <input className="form-check-input theme-img"
                type="radio"
                name="bgimg-radio" id={image.id}
                onChange={() => {
                    onChange(image);
                }}
                checked={checked}
            ...
            <label className="form-check-label avatar-xs" htmlFor={image.id}>
                <span className={
                    classnames("avatar-title", "rounded-circle", "theme-btn", [image.pattern], [image.id])
                }
                    id={`image-${image.id}`}
                >
                </span>
...
const ThemeImage = ({ theme }: ThemeImageProps) => {
    ...
    useEffect(() => {
        if (theme && theme.image) {
            const userTheme = themeImages.find((c: ThemeImageTypes) => c.id === theme.image);
            if (userTheme) {
